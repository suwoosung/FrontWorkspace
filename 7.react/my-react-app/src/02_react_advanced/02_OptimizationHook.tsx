...
export default function OptimizationHook(){
    /*
        #1. useMemo
         - 계산결과를 메모해두는 훅 함수. 
         - 리렌더링시 계산비용이 큰 작업의 결과를 캐싱하여 렌더링을 최적와
         한다. 
   ...
   const heavyComputation = (list: number[]) => {
    ...
    // 컴포넌트가 리렌더링 될 떄마다 정려뢴 배열을 다시 계산한다. 
    // const sorted = heacyComputation(numbers);

    // useMemo를 활용한 배열 캐싱
    // 현재 컴포넌트가 레너링 될때마다 배열을 정렬시키고 있는데,
    ...
    /*
        #2. useCallback
         ...
         - 함수를 새롭게 생성되면서 자식 컴포넌트 또한 리 렌더링 되는데 이를
         방지하기 위해 사용한다. 
         - React.memo와 함께 사용
    */
