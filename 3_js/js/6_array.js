test1.onclick = function(){
    /*
        자바스크립트의 배열은 크기제한이나, 타입제한이 없으며 유용한 메서드가
        존재한다(ArrayList와 비슷)
    */
    var arr1 = []; // 권장
    var arr2 = new Array();
...
/*
    push : 배열 마지막에 요소르 추가한 후, 변경된 길이를 반환
...
test12.onclick = function(){
    var hobbyies = prompt("취미를 입력하세요..");

    var arr = hobbyies.split(",");
    arr.sort();
...
test15.onclick = function(){
    // 1. foreach
    var arr = document.querySelectorAll("#drink li");
    var drink = [];
    arr.forEach(function(li){
        drink.push(li.innerText);
    });
    console.log(drink);
    // 2. find
    ...
    console.log(findOne);
    // 3. filter
    var filteredArray = drink.filter(function(d){
        return d.includes("o");
    });
    console.log(filteredArray);
    // 4. map
